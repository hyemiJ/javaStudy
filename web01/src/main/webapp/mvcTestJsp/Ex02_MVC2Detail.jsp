...
<body>
	...
	<c:if test="${not empty s}">
		<ul style="list-style:none;">${s.name}
			<li>\${s.sno} 학생번호 = <span class="red"> ${s.sno}</span></li>
			<li>\${s.age} 나이 = <span class="red"> ${s.age}</span></li>
			<li>\${s.jno} 조번호 = <span class="red"> ${s.jno}</span></li>
			<li>\${s.info} 자기소개 = <span class="red"> ${s.info}</span></li>
			<li>\${s.point} 점수 = <span class="red"> ${s.point}</span></li>
			<li>\${s.birthday} 생일 = <span class="red"> ${s.birthday}</span></li>
			<li>\${s.now} 가입날짜 = <span class="red"> ${s.now}</span></li>
		</ul>
	</c:if>
	<c:if test="${empty s}">출력할 정보가 없습니다.</c:if>
	<hr
		style="border-width: 1px 0 0 0; border-style: dashed; border-color: deepskyblue; width:700px;">
	<div>
