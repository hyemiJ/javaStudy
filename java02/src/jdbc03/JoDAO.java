...
public class JoDAO {
	...
	public List<JoDTO> joinTest(){
		sql = "SELECT j.jno , j.jname , captain , s.name , s.age , project , slogan "
				+"FROM jo j JOIN student s ON j.captain = s. sno";
		...
		try {
			...
			if (rs.next()) {
				do {
					...
					joinDto.setCaptain(rs.getInt(3));
